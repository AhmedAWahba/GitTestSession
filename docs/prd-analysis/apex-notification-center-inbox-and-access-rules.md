...
- **Scenarios authored:** broken down into 10 files under `scenarios/vendorApp/` (8 mapped to Linear `NC-` tickets plus 2 covering content types found only in the prototype), restyled to match `scenarios/access_management/business-invitation-accepting.feature`'s tagging and section conventions (no ticket tags; Linear/PRD sources noted as comments; no header before the first scenario block):
  - `notifications-inbox-and-bell-test.feature` (NC-3, ID-311) — reachability, bell badge, ordering, read/unread state, mark-as-read/mark-all-as-read (including a full-read assertion, not just the count), empty state, the no-destination-link edge case, and the single-list-vs-"Unread/Earlier" conflict
  - `notifications-multi-business-scoping-test.feature` (NC-1, ID-309) — per-business inbox isolation, independent unread counts, differing per-person unread counts on a shared notification (NC-1.12), cross-device read state, non-propagation of read state to co-recipients, access-ended and cross-business-authorization negatives
  ...
  - `notifications-account-and-role-triggers-test.feature` (NC-4, ID-312) — own-account notifications reaching every role, role-change email wording, Owner/Admin vs. Member business-notification receipt, plus a candidate outline for the still-unconfirmed "five account and role notifications" (now six candidates including "new device signed in")
  - `notifications-payment-request-expiry-reminder-test.feature` (NC-5, ID-313) — an outline covering both the 3-day (ticket) and 1-day (prototype) candidate thresholds, plus negatives for already-settled requests and duplicate reminders
  ...
  - `notifications-people-and-partner-events-test.feature` (new) — member joined, member removed, partner connected, and the removed-partner no-link edge case, covering the PRD's "People and partners" screens category that the original 8 files did not address
  - `notifications-business-document-events-test.feature` (new) — invoice registration finished/failed, invoice overdue, financing approved, financing settled — content observed only in the prototype, flagged as unconfirmed against the catalog
...
- The Gherkin scenarios have been broken down into 10 files under `scenarios/vendorApp/notifications-*-test.feature` (see §12). A re-check against the PRD, its screens list, and the live prototype found two more gaps now covered: notification content types (partner/membership events, invoice and financing outcomes) that the original 8 files omitted, and a no-destination-link edge case that conflicts with NC-1.6. The pending-decision scenarios are marked with `# Note:` comments (not tags) and must be resolved with Product/Engineering before they can be run to a pass/fail result.
